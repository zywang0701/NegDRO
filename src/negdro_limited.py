...
def negDRO_limited(x_list, y_list, gamma, intercept=False, lr_b=0.01, lambda_reg=0.001,
                  worst_adjust=0.2, worst_adjust_final=0.01, threshold=None, grad_threshold=1e-6, 
    ...
    def run_negDRO_step(b, worst_adjust_step, step):
        if verbose:
    ...
    while True:
        ...
        if prev_num_below_threshold is not None:
            if num_below_threshold == prev_num_below_threshold:
                ...
                if verbose:
                    print(f"-->No change in below-threshold variables at step {step + 1}. Running final step with worst_adjust {worst_adjust_final}")
                break
        
        prev_num_below_threshold = num_below_threshold
        step += 1
    
    # Final step with worst_adjust_step = 0.01
    b, b_step, w, num_below_threshold, max_loss, mask = run_negDRO_step(b, worst_adjust_final, step+1)
    b_list.append(b_step)
    w_list.append(w)
    max_loss_list.append(max_loss)
